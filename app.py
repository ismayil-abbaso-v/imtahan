...
# 🎫 Açıq sualları parse edən funksiya (bilet üçün)
...
# 📋 Əsas menyu və funksiya səhifələri
    ...
    # Sol menyuda görünən rejim dəyişdirici
    ...
    # 3️⃣ Bilet İmtahanı
        ...
        uploaded_file = st.file_uploader("📤 Bilet formatında sualları olan .docx faylı yükləyin", type="docx")

        if uploaded_file:
            if "ticket_questions" not in st.session_state:
                st.session_state.ticket_questions = parse_open_questions(uploaded_file)

            if "ticket_sample" not in st.session_state or st.button("🎲 Bilet Dəyiş"):
                st.session_state.ticket_sample = random.sample(
                    st.session_state.ticket_questions, 
                    min(5, len(st.session_state.ticket_questions))
                )

            st.markdown("### 📋 Bilet Sualları")
            for i, question in enumerate(st.session_state.ticket_sample, start=1):
