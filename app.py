...
if menu == "📤 Variantları Qarışdır": st.title("📤 Sual Variantlarını Qarışdır") uploaded_file = st.file_uploader("Word (.docx) sənədini seç", type="docx") mode = st.radio("Rejim:", ["50 sual", "Bütün suallar"], index=0)
...
elif menu == "📝 İmtahan Rejimi": st.title("📝 Öz İmtahanını Yoxla") uploaded_file = st.file_uploader("📤 Word (.docx) faylını yüklə", type="docx") mode = st.radio("📌 Rejim seç:", ["50 random sual", "Bütün suallar"], index=0)
